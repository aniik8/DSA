...
//        System.out.println(longestCommonSubsequence2("abcde", "ace"));
        System.out.println(longestCommonSubstr("ABCDGH"
                , "ACDGHR", 6, 6));
    }
...
// so to reduce the function calls and reduce the complexity of the program, use memoization
    ...
    static int longestCommonSubstr(String S1, String S2, int n, int m) {
        ...
        return substringhelper(S1, S2, n, m, t, 0);
    }

    static int substringhelper(String S1, String S2, int n, int m, int[][] t, int count) {
        ...
        if (S1.charAt(n - 1) == S2.charAt(m - 1))
             t[n][m] = substringhelper(S1, S2, n - 1, m - 1,  t, count+1);
        else
            t[n][m] = Math.max(count,Math.max(substringhelper(S1, S2, n, m-1, t, 0), substringhelper(S1, S2, n - 1, m ,t, 0)));

        return Math.max(t[n][m], count);
    }
/*
* int longestCommonSubstr(String S1, String S2, int n, int m){
        return substringhelper(S1, S2, n, m, 0);
       }
    int substringhelper(String S1, String S2, int n, int m, int count){

        if(n == 0 || m == 0)
            return count;
        if(S1.charAt(n-1) == S2.charAt(m-1))
            return count = substringhelper(S1, S2, n-1, m-1, count+1);
        else
            return count = Math.max(substringhelper(S1, S2, n, m-1, 0), substringhelper(S1, S2, n-1,m, 0));
